...
const ColumnModule = async ({ courseid }: { courseid: string }) => {
  ...
  return (
    <div className={` justify-center items-center grid grid-cols-1 ${styles['ColumnModuleContainer']}`}>
      <div className={`h-full w-80 bg-purpleMainLight col-start-2 col-span-4 px-7 ${styles['ColumnModule']}`}>
        <Link href={`/course-review/${courseid}`}>
          <div className={`bg-whiteColumn my-5 py-3 mt-10 flex flex-nowrap w-full h-[4.5rem] content-center hover:bg-purpleMainLighter ${styles['CourseTitle']}`}>
        ...
        {course.modules.length > 0 ? (
          course.modules.map((module: any, index: any) => (
            <Link href={`/module/${module.id}`} key={index}>
              <div className={`bg-whiteColumn mb-4 hover:bg-purpleMainLighter pl-5 pr-5 py-3 w-full h-[4.5rem] content-center cursor-pointer ${styles['ModuleButton']}`}>
