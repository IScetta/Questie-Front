...
const CreateCourse: React.FC<{ params: { slug: string } }> = ({ params }) => {
  ...
  const hasAccess = token && (payloadParsed?.isAdmin === "admin" || payloadParsed?.role === "admin");

  if (!hasAccess) {
    return (
      <div className="flex flex-col justify-center items-center">
        <h1 className="text-xl">No tiene las Credenciales para Acceder al sitio.</h1>
        <Link className="flex justify-center items-center bg-yellowMain rounded-md text-purpleMain h-10 w-52 ml-7 text-lg mt-5" href="/">
          Volver
        </Link>
      </div>
    );
        ...
        <div className="flex flex-col">
          ...
          <div className="w-full m-6 p-4 bg-white rounded-xl border-2 shadow-[0_5px_15px_0px_#00000042]">
            ...
            {course?.modules.map((module, index) => (
              <div className="flex flex-col bg-purpleMainLighter rounded-xl border-2 border-purpleMain m-2 p-2" key={index}>
